...
# --- 1. SİSTEM AYARLARI ---
st.set_page_config(
    page_title="ZACHAİRA V26 CERTIFIED", 
    page_icon="🦅", 
    layout="wide", 
    initial_sidebar_state="expanded" 
)

# Responsive & Temiz CSS
st.markdown("""
<style>
...
def veri_getir(hisse, bar_sayisi, interval, period):
    try:
        ...
        # --- RSI HESAPLAMA (KESİN VE GÜVENLİ YÖNTEM) ---
        delta = df['Close'].diff()
        # .clip() yöntemi parantez hatasına neden olmaz, temizdir.
        ...
        # Sıfıra bölünme hatasını önle (Nadir de olsa olur)
...
# --- 4. GRAFİK MOTORU (PLOTLY) ---
def grafik_ciz(df, hisse, veri):
    # Layout Ayarları (Şeffaf Arka Plan)
    layout = go.Layout(
        ...
        height=500, # Mobil/PC Ortak Yükseklik
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df['SMA50'], line=dict(color='cyan', width=1), name='SMA50'))
...
# --- 5. ANALİZ MOTORU (GEOMETRİK KONTROL + SNIPER) ---
def analiz_yap(df, secilen_formasyonlar, tolerans, zaman_etiketi, tek_hisse_modu=False):
    ...
    # Sniper Modu Kıyağı: Tek hisse arıyorsa toleransı artır
    if tek_hisse_modu: tol_katsayi *= 1.25 
    ...
    if "Boğa Bayrak" in secilen_formasyonlar:
        ...
        # Zirveden ÖNCEKİ veriye bak (Geriye doğru tarama)
        veri_oncesi = son_veri.loc[:idx_tepe]
        
        if len(veri_oncesi) > 3:
            idx_dip = veri_oncesi['Low'].idxmin() # Zirveden önceki en düşük yer
            dip_fiyat = veri_oncesi.loc[idx_dip]['Low']
            direk = tepe_fiyat - dip_fiyat
            
            # Direk boyu mantıklı mı? (> %5)
            if direk > (dip_fiyat * 0.05):
    ...
    # KURTARICI (Tek Hisse Modunda Boş Dönmesin)
    ...
    if bulunan:
        ...
        # Noktalar (Hata vermemesi için genel görünümde sahte nokta ata)
        if "Genel" not in bulunan:
            ...
            points = {"t_start": df.index[-1], "t_peak": df.index[-1], "t_break": df.index[-1], "p_start": son['Close'], "p_peak": son['Close'], "p_break": son['Close']}
...
with st.sidebar:
    ...
    # 2. KAYNAK SEÇİCİ
    liste_modu = st.radio("Kaynak:", ["TEK HİSSE (Sniper)", "FAVORİLERİM", "TÜM HİSSELER", "BIST 30"])
    
    tek_hisse_aktif = False
    if liste_modu == "TEK HİSSE (Sniper)":
    ...
    bar_sayisi = st.slider("Grafik Derinliği", 50, 300, 150)
...
if btn_baslat:
    ...
    if not bulunanlar:
        if tek_hisse_aktif: st.error(f"❌ {temiz_hisseler[0]} verisi alınamadı.")
        else: st.warning("❌ Sonuç yok. Toleransı artır.")
    else:
        st.success(f"🎉 {len(bulunanlar)} Sonuç Bulundu!")
